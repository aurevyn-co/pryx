...
| Repo | URL | Purpose |
|------|-----|---------|
| Pryx-Rust (NOW ROOT) | `src/` | Main Rust codebase (moved from newkind/pryx-rust) |
| OLD Pryx (Go/TS) | `_legacy/apps/`, `_legacy/packages/` | Reference implementation |
| OpenClaw | `.temp_refs/openclaw/` | Multi-channel reference |
...
## 10. Task Creation Status (Updated 2026-02-15)

### Beads Tasks Created

**Total: 27 tasks** (MIG-001 done, MIG-002 to MIG-028 created)

| Priority | Count | Status |
|----------|-------|--------|
| HIGH (P0) | 7 | Created: MIG-002, MIG-003, MIG-004, MIG-006, MIG-010, MIG-021, MIG-023 |
| MEDIUM (P1) | 11 | Created: MIG-005, MIG-007, MIG-008, MIG-009, MIG-013, MIG-018, MIG-019, MIG-020, MIG-022, MIG-026, MIG-027 |
| LOW (P2) | 9 | Created: MIG-011, MIG-012, MIG-014, MIG-015, MIG-016, MIG-017, MIG-024, MIG-025, MIG-028 |

### Task IDs Reference

**HIGH Priority (P0):**
- pryx-pw1k.1: MIG-002 (models.dev)
- pryx-9vpv: MIG-003 (MCP Client)
- pryx-pw1k.3: MIG-004 (TUI)
- pryx-nht0: MIG-006 (Mesh)
- pryx-ak6j: MIG-010 (Agent Spawn)
- pryx-h7wm: MIG-021 (Keychain)
- pryx-yktr: MIG-023 (Approvals)

**MEDIUM Priority (P1):**
- pryx-ogcq: MIG-005 (Web Admin)
- pryx-y623: MIG-007 (Session Timeline)
- pryx-ckp6: MIG-008 (Cost Tracking)
- pryx-tcvm: MIG-009 (OAuth Device)
- pryx-gnft: MIG-013 (Token Refresh)
- pryx-60pq: MIG-018 (Clipboard)
- pryx-324p: MIG-019 (Screen)
- pryx-rxi1: MIG-020 (LSP)
- pryx-vqf6: MIG-022 (Audit)
- pryx-6bk1: MIG-026 (Routing)
- pryx-a1hh: MIG-027 (PKCE)

**LOW Priority (P2):**
- pryx-7rgp: MIG-011 (Desktop)
- pryx-8j4v: MIG-012 (Tray)
- pryx-d1mn: MIG-014 (Email)
- pryx-8i2w: MIG-015 (Signal)
- pryx-hewu: MIG-016 (Teams)
- pryx-mdpt: MIG-017 (Voice)
- pryx-26ih: MIG-024 (Device Mgmt)
- pryx-j8ln: MIG-025 (WebSocket)
- pryx-mwot: MIG-028 (Auto-Update)

---

## 11. Source Code Locations

### OLD Pryx (Go/TS/Rust - Moved to _legacy/)
```
_legacy/apps/runtime/    # Go runtime (82K LOC) - reference only
├── internal/
│   ├── llm/           # 84+ providers (models.dev)
│   ├── mcp/           # MCP client + bundled tools
│   ├── mesh/          # Multi-device sync
│   ├── agent/spawn/   # Sub-agent orchestration
│   ├── keychain/      # OS keychain integration
│   ├── constraints/   # Human-in-the-loop
│   ├── cost/          # Token tracking
│   ├── audit/         # Action logging
│   └── auth/          # OAuth device flow
_legacy/apps/tui/       # Solid/OpenTUI terminal UI
_legacy/apps/local-web/ # React admin panel
_legacy/apps/host/      # Tauri desktop wrapper
```

### NEW Pryx Rust (Rust - Now at Root)
```
src/                    # Rust source (moved from newkind/pryx-rust)
├── providers/         # 22+ providers (need models.dev)
├── channels/          # 8 channels (CLI, TG, Discord, etc.)
├── tools/             # Shell, file, memory, browser
├── memory/            # SQLite + FTS5 + Vector
├── security/          # Pairing, policy, secrets
├── agent/             # Agent loop (need spawning)
...
### Reference: OpenClaw (TypeScript)
```
.temp_refs/openclaw/src/
├── channels/          # 10+ channels (email, signal, teams, etc.)
├── agents/            # Multi-agent routing
└── voice/             # Voice wake + talk
```

### Reference: OpenCode (TypeScript)
```
.temp_refs/opencode/packages/opencode/src/
├── lsp/               # Language Server Protocol
├── session/           # SQLite session management
└── tool/              # Tool schemas (TypeBox)
```
