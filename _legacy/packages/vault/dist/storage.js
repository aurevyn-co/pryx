...
import { VAULT_FORMAT_VERSION, StorageError, FileNotFoundError, CorruptedVaultError, EntryNotFoundError, DuplicateEntryError, } from './storage-types.js';
export class VaultStorage {
    ...
    async save(filePath, vault, _password) {
        ...
        try {
            ...
            throw new StorageError(`Failed to save vault: ${error.message}`);
        }
    }
    async addEntry(vault, entryData, password) {
    ...
    async updateEntry(vault, id, updates, password) {
    ...
    async getEntry(vault, id, password) {
        ...
        try {
            ...
            throw new CorruptedVaultError(`Failed to decrypt entry ${id}: ${error.message}`);
        }
    }
    listEntries(vault) {
    ...
    async verifyIntegrity(vault, password) {
        const report = {
            valid: true,
            errors: [],
...
export function createVaultStorage(backupDir) {
