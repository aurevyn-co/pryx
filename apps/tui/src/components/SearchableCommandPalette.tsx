      ...
      <box flexDirection="column" flexGrow={1} overflow="scroll">
        {commandsWithIndices().map(({ cmd, globalIdx, category }, idx) => {
          const isFirstInCategory =
            idx === 0 || commandsWithIndices()[idx - 1]?.category !== category;
          ...
          return (
            <box flexDirection="column">
              ...
              >
                <box width={25}>
                  <text fg={isSelected ? palette.accent : palette.accentSoft}>{cmd.name}</text>
                </box>
                <box flexGrow={1}>
                  <text fg={isSelected ? palette.text : palette.dim}>{cmd.description}</text>
                </box>
                {cmd.shortcut && (
                  <box width={10}>
                    <text fg={isSelected ? palette.accentSoft : palette.dim}>{cmd.shortcut}</text>
                  </box>
                )}
              </box>
            </box>
