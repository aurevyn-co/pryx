...
const OPEN_SKILLS_SYNC_MARKER: &str = ".pryx-open-skills-sync";
const OPEN_SKILLS_SYNC_INTERVAL_SECS: u64 = 60 * 60 * 24 * 7;

/// A skill is a user-defined or community-built capability.
/// Skills live in `~/.pryx/workspace/skills/<name>/SKILL.md`
...
fn open_skills_enabled() -> bool {
    if let Ok(raw) = std::env::var("PRYX_OPEN_SKILLS_ENABLED") {
...
fn resolve_open_skills_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("PRYX_OPEN_SKILLS_DIR") {
    ...
    if !readme.exists() {
        std::fs::write(
            ...
            "# ZeroClaw Skills\n\n\
             ...
             ```bash\n\
             pryx skills install <github-url>\n\
             pryx skills list\n\
             ```\n",
        )?;
    }
        ...
        super::SkillCommands::Install { source } => {
            ...
            if source.starts_with("http") || source.contains("github.com") {
                ...
                if output.status.success() {
                    ...
                    println!("  Restart `pryx channel start` to activate.");
                } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.pryx");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.pryx/skills"));
    }
