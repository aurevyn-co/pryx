...
// in `~/.pryx/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.pryx/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(pryx_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: pryx_dir.join(".secret_key"),
            enabled,
        }
    }
