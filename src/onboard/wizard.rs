...
pub fn run_wizard() -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to Pryx — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 8, "Channels (How You Talk to Pryx)");
...
pub fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to Pryx)");
...
/// Use `pryx onboard` or `pryx onboard --api-key sk-... --provider openrouter --memory sqlite`.
/// Use `pryx onboard --interactive` for the full wizard.
...
pub fn run_quick_setup(
    ...
    let pryx_dir = home.join(".pryx");
    let workspace_dir = pryx_dir.join("workspace");
    let config_path = pryx_dir.join("config.toml");
    ...
    if api_key.is_none() {
        ...
        println!("    2. Or edit:           ~/.pryx/config.toml");
        println!("    3. Chat:              pryx agent -m \"Hello!\"");
        println!("    4. Gateway:           pryx gateway");
    } else {
        println!("    1. Chat:     pryx agent -m \"Hello!\"");
        println!("    2. Gateway:  pryx gateway");
        println!("    3. Status:   pryx status");
    }
    println!();
...
fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    let home = directories::UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    let default_dir = home.join(".pryx");
...
fn setup_provider() -> Result<(String, String, String)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("Pryx works with ANY API that speaks the OpenAI chat completions format.");
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how Pryx connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("Pryx uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("Pryx can encrypt API keys stored in config.toml.");
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how Pryx stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to Pryx from anywhere.");
        ...
        match choice {
            0 => {
                ...
                println!(
                    ...
                    style("— talk to Pryx from Telegram").dim()
                );
            ...
            1 => {
                ...
                println!(
                    ...
                    style("— talk to Pryx from Discord").dim()
                );
            ...
            2 => {
                ...
                println!(
                    ...
                    style("— talk to Pryx from Slack").dim()
                );
            ...
            3 => {
                ...
                print_bullet("Pryx reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            5 => {
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("pryx-whatsapp-verify".into())
                    .interact_text()?;
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("Pryx is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if has_channels {
        ...
        println!("       {}", style("pryx channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("pryx agent -m \"Hello, Pryx!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("pryx agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("pryx status").yellow());
...
mod tests {
    ...
    fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = fs::read_to_string(tmp.path().join("IDENTITY.md")).unwrap();
        assert!(
            identity.contains("**Name:** Pryx"),
            "should default agent name to Pryx"
        );
