...
pub fn build_system_prompt(
    ...
    if prompt.is_empty() {
        "You are Pryx, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct.".to_string()
    } else {
        prompt
    }
}
...
pub fn handle_command(command: super::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        super::ChannelCommands::List => {
            ...
            println!("\nTo start channels: pryx channel start");
            println!("To check health:    pryx channel doctor");
            println!("To configure:      pryx onboard");
            Ok(())
        }
        super::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `pryx onboard` to configure channels"
            );
        }
        super::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.pryx/config.toml directly");
        }
    }
}
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `pryx onboard` first.");
        return Ok(());
    }

    println!("🩺 Pryx Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `pryx gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `pryx onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 Pryx Channel Server");
    println!("  🤖 Model:    {model}");
    println!(
        "  🧠 Memory:   {} (auto-save: {})",
...
mod tests {
    ...
    fn prompt_injects_workspace_files() {
        ...
        assert!(prompt.contains("Name: Pryx"), "missing IDENTITY content");
