...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            workspace_dir: pryx_dir.join("workspace"),
            config_path: pryx_dir.join("config.toml"),
            api_key: None,
...
impl Config {
    pub fn load_or_init() -> Result<Self> {
        ...
        if !pryx_dir.exists() {
            fs::create_dir_all(&pryx_dir).context("Failed to create .pryx directory")?;
            fs::create_dir_all(pryx_dir.join("workspace"))
...
"#;
    ...
    fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("pryx_test_config");
