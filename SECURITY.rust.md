...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/theonlyhennygod/pryx/security/advisories/new)
...
Pryx implements defense-in-depth security:
...
Pryx Docker images follow CIS Docker Benchmark best practices:
...
docker build -t pryx .
docker inspect --format='{{.Config.User}}' pryx
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace pryx gateway
```
